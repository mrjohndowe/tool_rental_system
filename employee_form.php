...
if ($_SERVER['REQUEST_METHOD']==='POST') {
 try {
  $name=trim($_POST['name']??''); $department=trim($_POST['department']??''); $badge=trim($_POST['badge_number']??''); $active=isset($_POST['active'])?1:0;
  if ($name==='' || $department==='') throw new RuntimeException('Employee name and department are required.');
  if ($id) { $s=$pdo->prepare('UPDATE employees SET name=?,department=?,badge_number=?,active=? WHERE id=?'); $s->execute([$name,$department,$badge?:null,$active,$id]); $newId=$id; }
  else { $s=$pdo->prepare('INSERT INTO employees(name,department,badge_number,active) VALUES(?,?,?,?)'); $s->execute([$name,$department,$badge?:null,$active]); $newId=(int)$pdo->lastInsertId(); }
  flash('success','Employee saved.');
  redirect($returnTo==='checkout' ? 'checkout.php?employee_id='.$newId : 'employees.php');
 } catch (Throwable $e) { flash('error', str_contains($e->getMessage(),'Duplicate') ? 'That badge number is already in use.' : $e->getMessage()); redirect('employee_form.php'.($id?'?id='.$id:'')); }
}
?>
