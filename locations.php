...
$edit=null;
if($id){$s=$pdo->prepare('SELECT * FROM tool_locations WHERE id=?');$s->execute([$id]);$edit=$s->fetch();}
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();flash('error',$e->getMessage());redirect('locations.php'.($id?'?id='.$id:''));}
}
...
?>
<div class="grid two">
<div class="card"><h2><?= $edit?'Edit':'Add' ?> Tool Location</h2><form method="post"><?php if($edit):?><input type="hidden" name="id" value="<?=$id?>"><?php endif;?>
...
<div class="actions"><button>Save Location</button><?php if($edit):?><a class="button secondary" href="locations.php">Cancel</a><?php endif;?></div></form></div>
<div class="card"><h2>Saved Locations</h2><div class="table-wrap"><table><thead><tr><th>Location</th><th>Shelf</th><th>Assigned</th><th></th></tr></thead><tbody>
<?php foreach($rows as $r):?><tr><td><strong><?=e($r['location_name'])?></strong><br><span class="muted"><?=e($r['area']?:'No area specified')?><?=$r['active']?'':' · Inactive'?></span></td><td><?=e($r['shelf'])?></td><td><?=(int)$r['tool_count']?> tools<br><?=(int)$r['accessory_count']?> accessories</td><td><a class="button secondary" href="locations.php?id=<?=(int)$r['id']?>">Edit</a></td></tr><?php endforeach;?>
<?php if(!$rows):?><tr><td colspan="4">No locations have been added.</td></tr><?php endif;?></tbody></table></div></div></div>
