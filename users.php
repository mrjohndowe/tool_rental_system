...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        flash('error', $message);
        redirect('users.php' . ($editId ? '?edit=' . $editId : ''));
    }
}

$editing = null;
if ($editId) {
    $s = $pdo->prepare('SELECT id,full_name,username,role,active FROM users WHERE id=?');
    $s->execute([$editId]);
    $editing = $s->fetch();
}
...
<div class="grid two">
    <div class="card">
        ...
        <form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="id" value="<?= (int)($editing['id'] ?? 0) ?>"><label>Full Name</label><input name="full_name" required value="<?= e($editing['full_name'] ?? '') ?>"><label>Username</label><input name="username" required autocomplete="off" value="<?= e($editing['username'] ?? '') ?>"><label>Password <?= $editing ? '<span class="muted">(leave blank to keep current password)</span>' : '' ?></label><input type="password" name="password" autocomplete="new-password" <?= $editing ? '' : 'required' ?>><label>Role</label><select name="role">
                <option value="clerk" <?= ($editing['role'] ?? '') === 'clerk' ? 'selected' : '' ?>>Clerk — issue and return tools</option>
                <option value="admin" <?= ($editing['role'] ?? '') === 'admin' ? 'selected' : '' ?>>Administrator — includes user management</option>
            </select>
            <div class="actions"><button>Save User</button><?php if ($editing): ?><a class="button secondary" href="users.php">Cancel</a><?php endif; ?></div>
        </form>
    </div>
    <div class="card">
        <h2>Access Levels</h2>
        <p><strong>Clerks</strong> can use the dashboard, check tools in and out, and manage employees and tools.</p>
        <p><strong>Administrators</strong> have all clerk access and can create, edit, activate, and deactivate login accounts.</p>
    </div>
</div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tbody><?php foreach ($users as $u): ?><tr>
                        ...
                        <td><?= $u['last_login_at'] ? e(date('M j, Y g:i A', strtotime($u['last_login_at']))) : 'Never' ?></td>
                        <td><span class="badge <?= $u['active'] ? 'available' : 'maintenance' ?>"><?= $u['active'] ? 'Active' : 'Inactive' ?></span></td>
                        <td>
